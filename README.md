...
**SAVE PROJECT** asks for an exact user-supplied name on the first save, then stores the named project in the IndexedDB Project Library under an independent stable ID. Further edits autosave after a short debounce; an unsaved project never becomes a silent `Untitled` entry. The library supports NEW, OPEN, RENAME, DUPLICATE, DELETE, EXPORT and IMPORT while preserving creation identity and timestamps appropriately. Opening a project after **START AUDIO** re-decodes local WAV data, regenerates waveform caches, restores samples, MONO-3, MONOGORG, STRINGS and POLY patches and chord assignments, Smart Chords performance settings, all Pattern Group banks, Playlist, TRACKS, mixer, Pump, FX racks, DRUM SYNTH and Project Key settings, and leaves transport stopped. Older project-state schemas retain their established migrations, and the previous single `default-project` save remains recoverable as unnamed legacy state until the user gives it a name.

Portable `.station` files are versioned JSON envelopes. This first portable version exports projects made from Station instruments and stable built-in-library asset IDs; it deliberately blocks export when imported, TRACKS, CHOP or generated audio is present because external WAV packaging is not implemented yet. Import validates both the file schema and musical-state schema. An existing `projectId` requires an explicit OPEN AS COPY or REPLACE decision, with copy as the safe first action. Local projects may still contain external WAVs because IndexedDB keeps their blobs; the portability restriction applies only to `.station` export. IndexedDB quota remains browser-managed, so saving large local WAV projects can fail when local storage is full.

### Current limitations

Projects remain local to the current browser profile, and the app does not auto-open the last one after reload. BASSIC, MONOGORG, STRINGS and ZOLA-X each have a separate device-local USER PRESETS library that survives reloads and project changes; loading one replaces only the current patch's sound parameters while preserving its project identity and shared-pad references. Each Pattern Group and the master have two serial insert FX slots, supporting NONE, compressor, BPM-synced delay and a three-band EQ (low shelf, mid peak, high shelf). MONO-3 is intentionally limited to one shared patch source per assigned pad, one filter LFO and five voices per patch; it has no Piano Roll, MIDI, modulation matrix or wavetable import. MONOGORG is always mono and intentionally exposes only its nine bass-focused controls, with no chord mode or modulation matrix. STRINGS is intentionally limited to one shared patch source per assigned pad, sawtooth-only oscillators, a fixed-Q filter, no filter envelope, no MONO mode and eight voices per patch; it has no Piano Roll, MIDI, sampled/multisampled strings, physical modeling or mod matrix. POLY ships with a fixed procedural bank and factory starting patches; user wavetable import, MPE and macros remain out of scope. Audio must be explicitly started after each page reload. If a browser suspends or interrupts Web Audio after a tab or app switch, Station attempts to resume when the page becomes visible and retries on the next pointer or keyboard gesture; browser policy may still require pressing **START AUDIO**.
