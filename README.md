...
**TUNE** is a temporary QUALITY research workspace for short monophonic vocal experiments. It can record or load a WAV, render four comparison variants in Web Workers, run a blind A–D listening test, export diagnostics and measure the offline workflow on a phone. It is not an accepted Station effect: Tune Gravity is absent from the FX Rack, project schema, SONG render and realtime audio path, and its quality on real vocals remains unverified. See [Tune Gravity DSP prototype](docs/TUNE_GRAVITY_DSP_PROTOTYPE.md).
