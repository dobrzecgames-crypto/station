...
**SAVE PROJECT** stores one local project in IndexedDB: its schema-v18 manifest and each referenced source WAV under a stable asset ID. **OPEN PROJECT** restores the last saved project after **START AUDIO**; it re-decodes WAV data, regenerates waveform caches, restores samples, MONO-3 and STRINGS patches and chord assignments, all Pattern Group banks, Playlist, mixer, Pump, FX racks and Project Key settings, and leaves transport stopped. Schema-v1 through v17 projects migrate safely with neutral defaults for fields and effect configs introduced later; their existing sound is unchanged.
