...
A Pattern Group is one musical idea. It owns one private 16-pad bank, a `padMode` (`notes` or `chords`) and 16 independent `chordAssignments`, and has an always-present A variant with optional B, C and D variants. Assignments store harmonic construction types relative to each pad's own scale-mapped root and register, rather than absolute notes. They are distinct from the older per-pad `chordIntervals` used by the MONOPOLY/STRINGS sound editor. Each existing variant is a complete 16-pad, 16-step pattern with a velocity and a SHIFT value for each step. SHIFT is constrained to −50% through +50% of one 16th-note duration. Creating a variation copies steps, velocity and SHIFT only; it never copies or changes the group's bank or pad mode. The A–D limit is intentional.
...
ProjectState schema v19 is the serializable boundary. Each Pattern Group contains its private bank, CHOP session, MONO-3 patch collection, STRINGS patch collection, `padMode`, 16 chord assignments and four-slot serial FX Rack. It contains shared asset references and durations, Pattern Groups and their 16-step velocities, selected Pattern Group/variant, Pattern Clips, persisted transport mode and Loop Song, BPM, swing, group-and-pad Pump references, the global Project Key (`root`, `scale`) and a serializable four-slot master FX Rack. Waveform peaks, active chord tokens and synth/strings voices are runtime cache/state and are not serialized.

Project Key remains a preference for Project Scale mapping in NOTES. A mapping writes normal independent pad configurations that reference one shared source and contain their calculated pitch offsets. Existing NOTES mappings are not retuned when it changes. CHORDS is the explicit exception: chord roots and compatibility follow the current Project Key, preserving a stored chord type when it remains diatonic and replacing it when it does not.
...
The one saved local project has ID `default-project`. Its manifest contains `ProjectState` with `schemaVersion: 19`; a separate asset record contains each referenced asset ID, filename, MIME type, byte size and WAV Blob imported from disk, generated by DRUM SYNTH or captured from the microphone. A shared asset is represented by one asset record even when referenced by many group banks. `lastProjectId` points to the most recently saved project. Schema-v1 through v18 migrations preserve existing sound and add `padMode: "notes"` plus 16 empty chord-assignment slots when those fields are absent. Missing assignments are generated only when that Pattern is first switched to CHORDS.
