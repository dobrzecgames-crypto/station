...
The legacy saved local project has ID `default-project`; current named projects use independent generated IDs. A manifest contains `ProjectState` with `schemaVersion: 22`; a separate asset record contains each referenced stable asset ID, filename, MIME type, byte size and original WAV Blob. A shared asset is represented by one asset record even when referenced by many group banks or projects. Stable asset IDs are immutable blob identities: introducing/replacing audio creates a new ID, and normal manifest autosaves leave an existing ID's Blob untouched. `lastProjectId` points to the most recently saved project. Historical migrations preserve the older sample/synth/pattern/mixer state. Schema-v20 migration adds an empty `polyPatches` collection to every group and `polyPatchId: null` to every pad, leaving all existing sources unchanged. Schema-v21 projects receive default Smart Chords performance settings without regenerating chord assignments or banks.
