...
RENDER in the PROJECT System Display panel writes SONG plus the intersecting TRACKS arrangement to a 16-bit stereo WAV file. The render is a second `AudioEngine` built over an `OfflineAudioContext` and driven by the same `StepSequencer` and `TimelineScheduler` entry points as live playback: the sequencer's wake-up is injected, so live playback wakes on a timer and a render wakes on the render clock through `OfflineAudioContext.suspend()`. TRACKS' bounded clip list is scheduled at render time zero through its normal scheduler, avoiding two clients competing for the same offline suspend quantum. Real-time capture through `MediaRecorder` was rejected because Chrome offers no WAV container, so every export would pass through Opus, and because a render taking as long as the song cannot survive a phone locking its screen.
...
- the transport and the render both read `getSongTracksForSlot`; TRACKS uses the same `toTimelineSchedulerClips` → `TimelineScheduler.scheduleClip` path in both,
- a render is always one pass of the playlist from slot 1 to the last occupied slot; every TRACKS clip starting before that final grid boundary is included and a crossing clip is stamped to stop there, while later clips are excluded exactly as they are from live SONG playback. LOOP SONG is a monitoring preference and PATTERN mode is not exported, since bouncing a loop belongs to M10 resampling, which returns to a pad instead of leaving as a file,
- the render length is computed rather than guessed: the longest pattern sample/synth release, serial delay/TIGHT ROOM tails through the active upstream and master racks, and a safety margin, with tails capped at twelve seconds and a ten-minute ceiling that fails with a message instead of exhausting a phone's memory,
...
- a missing decoded asset fails before creating the offline graph rather than producing a partial file; a ZOLA-X worklet failure fails only renders that actually use ZOLA-X and leaves core live audio available,
...
- WAV export (`RENDER`, DEC-021) includes TRACKS clips that intersect its explicit slot-1-to-final-SONG-slot interval through the same `TimelineScheduler`/track-bus path as live playback,
...
**Source-region export bridge.** A later WAVES/LASER workflow addition exposes the existing non-destructive clip region as reusable source material. Track Editor renders the full asset as a dedicated source-selection waveform: dragging across it creates a new region, while large orange START/END grips and sliders refine the same `sourceOffsetSeconds`–`sourceEndSeconds` values. A bordered bright selection and strongly dimmed discarded sides make the exported interval explicit; timeline clips retain always-visible edge grips for direct arrangement trimming. This source selection keeps a normal clip anchored on the timeline and updates its length to the selected duration; a looped clip retains its independent timeline footprint. The selected clip can download that interval as 16-bit PCM WAV or send the same WAV directly through LASER's existing `loadChopSourceBlob` path. The encoder accepts an exclusive `endFrame`, so a five-minute decoded asset does not need to be copied into a second full-size `AudioBuffer` merely to export thirty seconds. This is deliberately a raw crop rather than a track bounce: timeline placement, loop length, gain, fades, pitch, reverse, track bus gain and effects are not rendered. The direct path creates an ordinary persisted LASER asset and leaves the original track asset untouched; it does not add resampling, a second asset registry or TRACKS content to SONG RENDER.
