...
- if the pattern or colours need to change again, edit the `$rows` data (and `$gridSize`/`$origin`/`$pitch`/`$cell` if the grid itself changes) in `tools/icon/build-icon.ps1` and rerun it - it overwrites all four shipped sizes plus the reference render in one pass.
