...
export function LibraryWorkspace({ audioReady, busySampleId, previewingSampleId, onPreview, onLoad }: LibraryWorkspaceProps) {
  ...
  return (
    <section className="library-workspace" aria-labelledby="library-title">
      <div className="sequencer-heading"><div><p className="eyebrow">LIBRARY</p><h2 id="library-title">DRUMS</h2></div><p className="mixer-summary">PREVIEW · LOAD TO PAD</p></div>
      <p className="library-help">Tap a sound to preview it, then tap LOAD TO PAD and choose PAD 1–16. Right-click still opens the same menu on desktop.</p>
        ...
        {libraryCategories.map((category) => <section className="library-category" key={category} aria-label={`${category} sounds`}>
          ...
          {builtInLibrary.filter((sample) => sample.category === category).map((sample) => <article className="library-row" key={sample.id} onContextMenu={(event) => openLoadMenu(event, sample)}>
            <div className="library-row-actions">
              ...
              <button className="library-load-button" type="button" disabled={!audioReady || isBusy} onClick={() => setContextSample(sample)}>LOAD TO PAD</button>
            </div>
            {contextSample?.id === sample.id && <div className="library-load-menu" role="dialog" aria-label={`Load ${sample.filename} to pad`}>
              <label>LOAD TO PAD <input aria-label="Pad number" type="number" inputMode="numeric" min="1" max="16" value={targetPadNumber} onChange={(event) => setTargetPadNumber(Math.min(16, Math.max(1, Number(event.target.value))))} /></label>
