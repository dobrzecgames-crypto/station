...
interface MixerProps {
  ...
  ghostPumpSourceIds: readonly string[]
  pumpTargets: readonly string[]
  onGhostPumpSourceChange: (padId: PadState['id'], enabled: boolean) => void
...
export function Mixer({ pads, ghostPumpSourceIds, pumpTargets, onGhostPumpSourceChange, onVolumeChange, onMutedChange, onSoloChange }: MixerProps) {
  return (
    <section className="mixer" aria-labelledby="mixer-title">
      ...
      <div className="mixer-grid">
        {pads.map((pad) => {
          const isGhostPumpSource = ghostPumpSourceIds.includes(pad.id)
          ...
          return (
            <article className="mixer-channel" key={pad.id}>
              ...
              <p className="mixer-pump-status">{isGhostPumpSource ? 'GHOST SIDECHAIN' : isPumpTarget ? 'PUMP TARGET' : '-'}</p>
              ...
              <div className="mixer-buttons">
                ...
                <button className={isGhostPumpSource ? 'mixer-toggle mixer-toggle-active' : 'mixer-toggle'} type="button" aria-pressed={isGhostPumpSource} onClick={() => onGhostPumpSourceChange(pad.id, !isGhostPumpSource)}>SIDECHAIN</button>
              </div>
            </article>
          )
        })}
