...
export const chordTypes = [
  ...
  // Sparse scales have a few degrees for which none of the requested named
  // constructions is diatonic. A root doubled at the octave keeps every pad
  // playable without silently introducing an out-of-scale fifth or third.
  'rootOctave',
] as const
...
export function chordSuggestions(group: PatternGroup, pad: PadState, projectKey: ProjectKey): ChordSuggestion[] {
  ...
  return compatibleDefinitions(pad, projectKey)
    .map((definition) => suggestion(definition, rootMidi))
}
...
function suggestion(definition: ChordDefinition, rootMidi: number): ChordSuggestion {
  ...
  return {
    ...
    name: `${rootName}${definition.suffix}`,
    intervals: definition.intervals,
    midiNotes,
    noteNames: midiNotes.map((note) => noteNames[modulo12(note)]),
  }
}
