...
/** The migration boundary shared by project loading and cloning. Only absent
    fields receive legacy defaults; malformed present fields survive for the
    project validator to reject rather than being silently rewritten. */
...
export function chordFieldsForMode(pads: readonly PadState[], current: PatternChordFields, mode: PadMode, projectKey: ProjectKey): PatternChordFields {
  return {
    padMode: mode,
    chordAssignments: mode === 'chords'
      ? createChordAssignments(pads, projectKey, current.chordAssignments)
      : cloneAssignments(current.chordAssignments),
  }
}
...
export function repairedChordFields(pads: readonly PadState[], current: PatternChordFields, projectKey: ProjectKey): PatternChordFields {
  if (current.padMode === 'notes' && current.chordAssignments.every((assignment) => assignment === null)) {
    return { ...current, chordAssignments: cloneAssignments(current.chordAssignments) }
  }
