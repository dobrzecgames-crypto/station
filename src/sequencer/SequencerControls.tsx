...
import type { PatternGroup, PatternRetriggerMode, PatternVariantName, StepPattern, StepShiftPattern } from '../patterns/patternTypes'
import type { PadState } from '../pads/types'
import { Waveform } from '../sample-editor/Waveform'

export type RecordQuantization = '1/4' | '1/8' | '1/16'
...
interface SequencerControlsProps {
  ...
  recording: boolean
  recordQuantization: RecordQuantization
  retriggerMode: PatternRetriggerMode
  ghostPumpSourceIds: readonly PadState['id'][]
  ...
  onRecordQuantizationChange: (quantization: RecordQuantization) => void
  onRetriggerModeChange: (retriggerMode: PatternRetriggerMode) => void
}

export function SequencerControls({ pattern, shifts, pads, selectedPadId, groups, selectedGroupId, selectedVariant, selectedPad, selectedPeaks, playheadSeconds, playingStep, recording, recordQuantization, retriggerMode, ghostPumpSourceIds, onSelectPad, onSelectGroup, onSelectVariant, onNewGroup, onDuplicateVariant, onClearVariant, onDeleteGroup, onToggleStep, onVelocityChange, onShiftChange, onRecordQuantizationChange, onRetriggerModeChange }: SequencerControlsProps) {
  ...
  return <section className="sequencer" aria-labelledby="sequencer-title">
    ...
    <div className="pattern-actions pattern-recording-controls"><span className={recording ? 'sequence-target sequence-target-recording' : 'sequence-target'}>{recording ? `REC ARMED / ${group.name} ${selectedVariant}` : 'RECORD QUANTIZE'}</span>{(['1/4', '1/8', '1/16'] as const).map((quantization) => <button className={recordQuantization === quantization ? 'mixer-toggle mixer-toggle-active' : 'mixer-toggle'} key={quantization} type="button" aria-pressed={recordQuantization === quantization} onClick={() => onRecordQuantizationChange(quantization)}>{quantization}</button>)}</div>
    <div className="pattern-actions"><span className="sequence-target">{group.name.toUpperCase()} RETRIGGER</span><button className={retriggerMode === 'layer' ? 'mixer-toggle mixer-toggle-active' : 'mixer-toggle'} type="button" aria-pressed={retriggerMode === 'layer'} onClick={() => onRetriggerModeChange('layer')}>LAYER</button><button className={retriggerMode === 'cut-previous' ? 'mixer-toggle mixer-toggle-active' : 'mixer-toggle'} type="button" aria-pressed={retriggerMode === 'cut-previous'} onClick={() => onRetriggerModeChange('cut-previous')}>CUT PREVIOUS</button></div>
    ...
    <div className="pattern-matrix" aria-label="16-step pattern matrix"><div className="pattern-matrix-row pattern-matrix-header"><span>PAD</span>{Array.from({ length: 16 }, (_, index) => <span className={playingStep === index ? 'pattern-step-playing' : ''} key={index}>{index + 1}</span>)}</div>{pads.map((pad) => { const isGhostPumpSource = ghostPumpSourceIds.includes(pad.id); return <div className="pattern-matrix-row" key={pad.id}><button className={pad.id === selectedPadId ? 'pattern-pad-label pattern-pad-selected' : 'pattern-pad-label'} type="button" onClick={() => onSelectPad(pad.id)}>{pad.label}<small>{pad.fileName ?? 'EMPTY'}{isGhostPumpSource ? ' · SC' : ''}</small></button>{pattern[pad.id].map((stepVelocity, stepIndex) => <button key={stepIndex} className={`step pattern-step ${stepVelocity ? 'step-active' : ''} ${stepVelocity === 1 ? 'step-full' : ''} ${isGhostPumpSource && stepVelocity ? 'pattern-step-ghost' : ''} ${editedStep.padId === pad.id && editedStep.stepIndex === stepIndex ? 'pattern-step-selected' : ''} ${playingStep === stepIndex ? 'pattern-step-playing' : ''}`} type="button" aria-label={`${pad.label}, step ${stepIndex + 1}${isGhostPumpSource ? ', ghost sidechain' : ''}`} aria-pressed={stepVelocity > 0} onClick={() => selectStep(pad.id, stepIndex)}><small>{stepVelocity ? `${Math.round(stepVelocity * 100)}%` : ''}</small></button>)}</div>})}</div>
