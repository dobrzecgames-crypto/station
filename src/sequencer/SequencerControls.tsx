...
interface SequencerControlsProps {
  ...
  pads: readonly Pick<PadState, 'id' | 'label' | 'fileName' | 'synthPatchId' | 'organicBassPatchId' | 'polyPatchId'>[]
  ...
  return <section className="sequencer" aria-label={`Sequencer, ${group.name} ${selectedVariant}`}>
    ...
    >
      ...
      {pads.map((pad) => {
        ...
        const sourceLabel = pad.synthPatchId ? 'BASSIC' : pad.organicBassPatchId ? 'MONOGORG' : pad.polyPatchId ? 'ZOLA-X' : pad.fileName ?? 'EMPTY'
