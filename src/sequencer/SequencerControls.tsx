...
export function SequencerControls({ pattern, shifts, pads, selectedPadId, groups, selectedGroupId, selectedVariant, onSelectPad, onSelectGroup, onSelectVariant, onNewGroup, onDuplicateVariant, onClearVariant, onDeleteGroup, onToggleStep, onVelocityChange, onShiftChange }: SequencerControlsProps) {
  ...
  return <section className="sequencer" aria-labelledby="sequencer-title">
    ...
    <div className="pattern-matrix" aria-label="16-step pattern matrix"><div className="pattern-matrix-row pattern-matrix-header"><span>PAD</span>{Array.from({ length: 16 }, (_, index) => <span key={index}>{index + 1}</span>)}</div>{pads.map((pad) => <div className="pattern-matrix-row" key={pad.id}><button className={pad.id === selectedPadId ? 'pattern-pad-label pattern-pad-selected' : 'pattern-pad-label'} type="button" onClick={() => onSelectPad(pad.id)}>{pad.label}<small>{pad.fileName ?? 'EMPTY'}</small></button>{pattern[pad.id].map((stepVelocity, stepIndex) => <button key={stepIndex} className={`step pattern-step ${stepVelocity ? 'step-active' : ''} ${stepVelocity === 1 ? 'step-full' : ''} ${editedStep.padId === pad.id && editedStep.stepIndex === stepIndex ? 'pattern-step-selected' : ''}`} type="button" aria-label={`${pad.label}, step ${stepIndex + 1}`} aria-pressed={stepVelocity > 0} onClick={() => selectStep(pad.id, stepIndex)}><small>{stepVelocity ? `${Math.round(stepVelocity * 100)}%` : ''}</small></button>)}</div>)}</div>
